...
def eval_jets_on_test_set(model):
    ...
    model_scores['ARI'] = np.vectorize(metrics.adjusted_rand_score)(target, pred)
...
def _predict_on_test_set(model):
    ...
    for tracks_in_jet in range(2, np.amax(n_tracks) + 1):
        ...
        for i in range(0, batch_len, 2048):
            if isinstance(model, SetPartitionTri):
                predictions += list(model(input_batch[i:min(i + 2048, batch_len)], None)[0].cpu().data.numpy())
            else:
                predictions += list(infer_clusters(
                    model(input_batch[i:min(i + 2048, batch_len)]).squeeze(1)).cpu().data.numpy())  # Shape
