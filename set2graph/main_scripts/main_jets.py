...
def parse_args():
    ...
    argparser.add_argument('--method', default='lin2',
                           help='Method to transfer from sets to graphs: lin2 for S2G, lin5 for S2G+')
    argparser.add_argument('--res_dir', default='../experiments/jets_results', help='Results directory')
    argparser.add_argument('--baseline', default=None,
                           help='Use a baseline and not set2graph. mlp, gnn, siam or siam3.')

    argparser.add_argument('--debug_load', dest='debug_load', action='store_true',
                           help='Load only a small subset of the data')
    ...
    # Transformer args
    argparser.add_argument('-he', '--n_heads', default=1, type=int, help='The number of heads')
    argparser.add_argument('-de', '--depth', default=1, type=int, help='The depth of the Transformer')
    argparser.add_argument('-ex', '--expansion', default=2, type=int, help='Expansion ratio for the Transformer')
    argparser.add_argument('-sw', '--share_s2g', action='store_true', help='Whether to share weights for S2G layers')
    argparser.add_argument('-ma', '--mask_scale', default=10000., type=float, help='The scale for the mask')

    args = argparser.parse_args()

    assert args.baseline is None or args.baseline in ['mlp', 'gnn', 'siam', 'siam3', 'dev', 'transformer']
...
def main():
    ...
    elif config.baseline == 'transformer':
        model = ControllerTransformer(depth=config.depth,
                                      expansion_ratio=config.expansion,
                                      n_heads=config.n_heads,
                                      s2g_sharing=config.share_s2g,
                                      in_features=10,
                                      out_features=1,
                                      set_fn_feats=[256, 256, 256, 5],
